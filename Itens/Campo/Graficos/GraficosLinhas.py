...
class GraficosLinhas(ft.UserControl):
    def __init__(self,page, anoSelect = '----', anoAnterior = '----'):
    ...
    def setValue(self,ano):
        ...
        dadosAno = self.dados.totalAnoConcluido(ano)
        dadosAnoAnterior = self.dados.totalAnoConcluido(anoAnterior)

        if dadosAno < 2000 and dadosAnoAnterior < 2000 :
            self.chart.max_y = 2000
            self.chart.left_axis.labels_interval = 1000
        elif dadosAno < 4000 and dadosAnoAnterior < 4000 :
            self.chart.max_y = 4000
            self.chart.left_axis.labels_interval = 1000
        elif dadosAno < 8000 and dadosAnoAnterior < 8000 :
            self.chart.max_y = 8000
            self.chart.left_axis.labels_interval = 1000
        elif dadosAno < 14000 and dadosAnoAnterior < 14000 :
            self.chart.max_y = 14000
            self.chart.left_axis.labels_interval = 2000
        else:
            self.chart.max_y = 20000
            self.chart.left_axis.labels_interval = 2000

        self.anoEscolhido.value = ano
        self.anoAnterior.value = anoAnterior        
    ...
    def setGrafico1(self,ano):
        valorMes = self.dados.valorMesesConcluido(ano)
        
        if len(self.data_1[0].data_points) != 0:
            self.data_1[0].data_points = []

        for i in range(0,12):

            try:
                self.data_1[0].data_points.append(ft.LineChartDataPoint(i, valorMes[self._selecionaMes(i)],tooltip_style=ft.TextStyle(weight=ft.FontWeight.BOLD)),)
            except KeyError:
                self.data_1[0].data_points.append(ft.LineChartDataPoint(i, 0,tooltip_style=ft.TextStyle(weight=ft.FontWeight.BOLD)),)
                

    def setGrafico2(self,ano):
        valorMes = self.dados.valorMesesConcluido(ano)
        
        if len(self.data_1[1].data_points) != 0:
            self.data_1[1].data_points = []

        for i in range(0,12):

            try:
                self.data_1[1].data_points.append(ft.LineChartDataPoint(i, valorMes[self._selecionaMes(i)],tooltip_style=ft.TextStyle(weight=ft.FontWeight.BOLD)),)
            except KeyError:
