...
class TodoApp(ft.UserControl):
    ...
    def build(self):
        self.menu = MenuPrincipal(self.cadastro, self.consultas,self.anotações,self.pagamentos)
    ...
    def consultas(self,e):
        '''Quando selecionado, abre o janela consultas'''
        #### Se houver uma janela já aberta, ele fecha a aberta e abre a janela selecionada. 
        if (len(self.desiner.controls) > 2):
            self.desiner.controls.pop(2)
            self.desiner.controls.append(ft.Column([ft.Icon(name=ft.icons.CONSTRUCTION, size= 200),ft.Text("Consultas... Em Desenvolvimento...",size=20)]))
